...
try {
    ...
    // Get Customer
    $api = API_PAYMENTS . "/" . $incoming->requestId . "/captures";
    $result = ProcessRequest(MID, $api , METHOD_POST, $requestBody, CHILD_MID, AUTH_TYPE_SIGNATURE );
    $dbError = "";
    if($result->responseCode == 201){
        if( !$dbResult = insertPayment($incoming->accessToken, "CAPTURE", $incoming->orderId, $incoming->amount, 0, $incoming->currency,
        ...
        if( !$dbResult = updateOrder($incoming->accessToken, $incoming->orderId, "CAPTURED")){
