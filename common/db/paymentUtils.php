...
function insertPayment($accessToken, $type, $orderId, $amount, $captured, $currency,
    ...
    $url = 'http://'. $_SERVER['SERVER_NAME'] . '/payPage/common/v1/controller/payments.php';
...
function updateOrder($accessToken, $id, $status){
    ...
    $url = 'http://'. $_SERVER['SERVER_NAME'] . '/payPage/common/v1/controller/orders.php?orderId='.$id.'&patch=';
    [$responseCode, $response] = fetch($accessToken, METHOD_POST, $url, json_encode($payload));
    if($responseCode != 200){
        http_response_code($responseCode);
        echo "Error: ". $response;
